...
PACKAGE_PATH_FILE=${CARLA_UNREAL_ROOT_FOLDER}/Content/PackagePath.txt
MAP_LIST_FILE=${CARLA_UNREAL_ROOT_FOLDER}/Content/MapPathsLinux.txt
...
for PACKAGE_NAME in "${PACKAGES[@]}" ; do if [[ ${PACKAGE_NAME} != "Carla" ]] ; then
  ...
  PACKAGE_PATH=${CARLA_UNREAL_ROOT_FOLDER}/Content/${PACKAGE_NAME}
  ...
  SUBST_FILE="${PACKAGE_FILE/${CARLA_UNREAL_ROOT_FOLDER}/${SUBST_PATH}}"
  ...
  for i in "${ADDR[@]}"; do # access each element of array
    ...
    XODR_FILE_PATH="${CARLA_UNREAL_ROOT_FOLDER}/Content${i:5}"
    MAP_NAME=${XODR_FILE_PATH##*/}
    XODR_FILE=$(find "${CARLA_UNREAL_ROOT_FOLDER}/Content" -name "${MAP_NAME}.xodr" -print -quit)

    if [ -f "${XODR_FILE}" ] ; then

      SUBST_FILE="${XODR_FILE/${CARLA_UNREAL_ROOT_FOLDER}/${SUBST_PATH}}"
    ...
    BIN_FILE_PATH="${CARLA_UNREAL_ROOT_FOLDER}/Content${i:5}"
    MAP_NAME=${BIN_FILE_PATH##*/}
    find "${CARLA_UNREAL_ROOT_FOLDER}/Content" -name "${MAP_NAME}.bin" -print0 | while read -d $'\0' BIN_FILE
    do
      if [ -f "${BIN_FILE}" ] ; then

        SUBST_FILE="${BIN_FILE/${CARLA_UNREAL_ROOT_FOLDER}/${SUBST_PATH}}"
