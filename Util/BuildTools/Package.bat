...
if not defined CARLA_UNREAL_ENGINE_ROOT (
    ...
    for /f "usebackq tokens=1,2,*" %%A in (`reg query !KEY_NAME! /s /reg:64`) do (
        if "%%A" == "!VALUE_NAME!" (
            set CARLA_UNREAL_ENGINE_ROOT=%%C
        )
    )
    if not defined CARLA_UNREAL_ENGINE_ROOT goto error_unreal_no_found
)
...
set BUILD_FOLDER=%INSTALLATION_DIR%CarlaUnreal/%CARLA_VERSION%/

set DESTINATION_ZIP=%INSTALLATION_DIR%CarlaUnreal/CARLA_%CARLA_VERSION%.zip
...
if %DO_PACKAGE%==true (
    ...
    call "%CARLA_UNREAL_ENGINE_ROOT%\Engine\Build\BatchFiles\Build.bat"^
        CarlaUnrealEditor^
        Win64^
        Development^
        -WaitMutex^
    ...
    echo "%CARLA_UNREAL_ENGINE_ROOT%\Engine\Build\BatchFiles\Build.bat"^
        CarlaUnreal^
        Win64^
        %PACKAGE_CONFIG%^
        -WaitMutex^
    ...
    echo "%CARLA_UNREAL_ENGINE_ROOT%\Engine\Build\BatchFiles\RunUAT.bat"^
        BuildCookRun^
        -nocompileeditor^
    ...
    call "%CARLA_UNREAL_ENGINE_ROOT%\Engine\Build\BatchFiles\RunUAT.bat"^
        BuildCookRun^
        -nocompileeditor^
        -TargetPlatform=Win64^
        -Platform=Win64^
...
set CARLA_UNREAL_ROOT_FOLDER=%ROOT_PATH%Unreal/CarlaUnreal
set PACKAGE_PATH_FILE=%CARLA_UNREAL_ROOT_FOLDER%/Content/PackagePath.txt
set MAP_LIST_FILE=%CARLA_UNREAL_ROOT_FOLDER%/Content/MapPaths.txt
...
for /f "tokens=* delims=" %%i in ("!PACKAGES!") do (
    ...
    if not !PACKAGE_NAME! == Carla (
        ...
        set BUILD_FOLDER=%INSTALLATION_DIR%CarlaUnreal/!PACKAGE_NAME!_%CARLA_VERSION%\
        set PACKAGE_PATH=%CARLA_UNREAL_ROOT_FOLDER%/Content/!PACKAGE_NAME!

        if not exist "!BUILD_FOLDER!" mkdir "!BUILD_FOLDER!"

        echo Cooking package '!PACKAGE_NAME!'...

        pushd "%CARLA_UNREAL_ROOT_FOLDER%"

        echo   - prepare
        REM # Prepare cooking of package
        echo Prepare cooking of package: !PACKAGE_NAME!
        call "%CARLA_UNREAL_ENGINE_ROOT%/Engine/Binaries/Win64/UnrealEditor.exe "^
        "%CARLA_UNREAL_ROOT_FOLDER%/CarlaUnreal.uproject"^
        ...
        if %DO_TARBALL%==true (
            ...
            if %SINGLE_PACKAGE%==true (
                ...
                set DESTINATION_ZIP=%INSTALLATION_DIR%CarlaUnreal/%TARGET_ARCHIVE%_%CARLA_VERSION%.zip
            ) else (
                echo Packaging '!PACKAGE_NAME!'...
                set DESTINATION_ZIP=%INSTALLATION_DIR%CarlaUnreal/!PACKAGE_NAME!_%CARLA_VERSION%.zip
            )
